    ...
    def get(self):
        ...
        for _ in range(50):
            ...
            items.append(SimpleNamespace(
                id=f'mock_chat_{self.msgs_generated}',
                timestamp=1234567890 + self.msgs_generated,
                datetime='2026-04-11 12:00:00',
                message=f'mock message {self.msgs_generated}',
...
def test_end_to_end_holodex_to_scrape(monkeypatch):
    ...
    if not mysql_host or not ho_api_key:
        pytest.skip('MySQL integration environment or Holodex API key not configured')
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        monkeypatch.setattr('scrape.pytchat.create', lambda video_id: MockPytchatVideo(video_id))

        configs = ConfigHandler(config_path)
        ...
        scraper = Scraper(video_id, max_messages=150, max_duration_seconds=10)
        scraper.run()
